        ...
        if(!$token->validateToken($POST['_once'], 300)){
            ...
            if(sizeof($error) === 0){
                /*if($teams->doExists($teamName)){
                    ...
                    if($teams->updateTeam($teamData)){
                        $success = 'Holdet er nu ændret!';
                    }
               // }
    ...
    <div class="col s6">
        ...
        <form action="" method="post">
            ...
            <div class="input-field col s12"> 
                <select id="style" name="style">
                    ...
                    <?php
                        ...
                        ?>
                            <option value="<?=$style->stylesId?>" <?=$team->fkStyle === $style->stylesId ? 'selected' : ''?>><?=$style->stylesName?></option>
                        <?php
                        }
                    ?>
                </select>
            ...
            <div class="input-field col s12"> 
                <select id="ageGruop" name="ageGroup">
                    ...
                    <?php
                        ...
                        ?>
                            <option value="<?=$ageGroup->ageGrpId?>" <?=$team->fkAgeGrp === $ageGroup->ageGrpId ? 'selected' : ''?>><?=$ageGroup->ageGrpName?></option>
                        <?php
                        }
                    ?>
                </select>
            ...
            <div class="input-field col s12"> 
                <select id="level" name="level">
                    ...
                    <?php
                        ...
                        ?>
                            <option value="<?=$level->levelId?>" <?=$team->fkLevel === $level->levelId ? 'selected' : ''?>><?=$level->levelName?></option>
                        <?php
                        }
                    ?>
                </select>
            ...
            <div class="input-field col s12"> 
                <select id="instructor" name="instructor">
                    ...
                    <?php
                        ...
                        ?>
                            <option value="<?=$instructor->insId?>" <?=$team->fkInstructor === $instructor->insId ? 'selected' : ''?>><?=$instructor->firstname . ' ' . $instructor->lastname?></option>
                        <?php
                        }
                    ?>
                </select>
            ...
            <div class="input-field col s12">
                ...
                <input type="number" min="1" step="any" name="teamPrice" id="teamPrice" value="<?=$team->teamPrice?>">
                <?= isset($error['teamPrice']) ? '<p class="red-text text-ligthen-2">' . $error['teamPrice'] . '</p>' : ''?>
            </div>
            <div class="input-field col s12">
                <button type="submit" class="btn" name="UpdateTeam">Ret<i class="material-icons right">send</i></button>
            </div>
        </form>
    </div>
</div>