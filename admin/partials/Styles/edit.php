<?php
    ...
    if(isset($GET['id']) && !empty($GET['id'])){
        $styleId = $GET['id'];
        $user->checkAuth(50, $_SESSION['userId']) ? null : header('Location: ./index.php?p=home&view=Styles/List');
    }else{
    ...
    if($filter->checkMethod('POST')){
        ...
        if(!$token->validateToken($POST['_once'], 300)){
            ...
            if(sizeof($error) === 0){
                ...
                if(!empty($_FILES['stylesPicture']['name'])){
                    $picture = $upload->uploadImage('stylesPicture');
                    if(!$picture['error']){
                        $styleData[':PICTURE'] = $picture['name'];
                        $styleData[':MTYPE'] = $picture['type'];
                    }else{
                        $error['stylesPicture'] = $picture['msg'];
                    }
                }
                if((sizeof($error) === 0) && $content->updateStyle($styleData)){
                    $success = 'Stilarten er nu blevet opdateret!';
                }
            }
        }
    }
...
<div class="row ">
    <div class="col s6">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <div class="input-field col s12">
                <button type="submit" class="btn" name="UpdateStyle">Ret<i class="material-icons right">send</i></button>
            </div>
        </form>
    </div>
</div>